 ...
 * @returns {Promise} 重试后的请求
 */
const retryRequest = async config => {
	const retryCount = config._retryCount || 0;
	if (retryCount >= 2) {
		ElMessage.error(`请求多次超时，请检查网络或稍后重试`);
		return Promise.reject(new Error(`请求超时且重试次数已达上限`));
	}
...
http.interceptors.response.use(
	...
	async error => {
		...
		if (isTimeoutError && config) {
			try {
				await ElMessageBox.confirm(`请求超时（已等待${config.timeout / 1000}秒），是否重试？`, '提示', {
				...
				return Promise.reject(new Error(`用户取消超时请求重试`));
			}
		}
