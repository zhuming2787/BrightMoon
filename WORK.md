...
## 5. 提交规范校验：用husky+commitlint约束 Git 提交信息格式
    ...
    1、安装一堆东西
    2、用处就是在github提交前会强制更新代码格式，非常好用，防止垃圾代码上传到github
...
## 6. 环境变量区分：配置.env.development/.env.production，区分开发 / 生产环境
    ...
    2、开发环境的接口地址主要是本地服务器
    3、开发环境默认开启调式模式
    4、通过import.meta.env访问 | process.env是vue2的

    结果：环境变量会被打包到前端代码中，绝对不能存放密钥、token等敏感数据

## 7. 组件自动注册：基于 Vite 的import.meta.glob，自动注册全局组件

    自动注册全局组件 ≠ 同步加载

    手动注册的繁琐：如果有10+全局组件，手动写import xxx from './xxx.vue'会重复几十行代码，新增/删除组件还要改注册代码，容易漏写、命名不规范。

    约定好目录/命名规则后，只需把全局组件放到指定目录，无需关心注册逻辑，避免人为错误。

    1、编写components/index.js，自动注册components下所有.vue文件，排除index.js自身，通过impoer.meta.glob批量匹配组件文件，并注册
    2、在main.js中引入上诉注册函数，传入Vue应用实例完成全局注册：
    3、仅注册指定规则的全局注册（避免注册局部组件）
    4、异步加载全局组件（优化首屏加载）

## 8. 错误边界：封装ErrorBoundary组件，捕获子组件渲染错误

    1、编写ErrorBoundary组件
    2、思路：将子组件（比如Card等易出错的组件）外层包一个ErrorBoundary
    3、本质上是在出错组件的上层封装一个鲁棒性好的，统一的错误组件

    结果：暂时只给@/home中的Card封装了一下

## 9. Mock 数据：集成vite-plugin-mock，本地模拟接口数据，无需依赖后端

    1、环境配置和mock配置可以上网搜索
    2、用法：在mock中直接创建对应API的接口，然后返回创建假数据返回，非常好用~
    3、生产环境会自动禁用mock

## 10. 打包体积分析：用rollup-plugin-visualizer分析打包体积，优化大依赖

    结果："build:analyze": "cross-env ANALYZE=true vite build" 创建dist包，同时创建一个旭日图，非常的直观。
...
## 1. 移动端适配方案：基于postcss-px-to-viewport实现 px 转 vw/vh
    ...
    PostCSS有什么用？
    1、基于设计稿宽度，将px转化为vw/vh
    2、自动添加浏览器厂商前缀（最经典功能）
    3、使用未来/现代 CSS 语法（今天就用明天的新特性）
    4、CSS 代码压缩与优化（生产环境必备）
    5、配置看配置文件postcss.config.js
...
## 7. 移动端复制功能：封装copyToClipboard函数，适配移动端剪贴板. 横屏 / 竖屏提示：监听屏幕旋转，提示用户竖屏使用（如表单场景）
...
## 1. 全局错误捕获：用app.config.errorHandler捕获 Vue 运行时错误
    ...
    1、利用vue3自带的createApp创建app
    2、然后紧跟着刚创建的app，就让它挂载errorHandler全局错误捕获
    3、如果有错误，可以先报给blog控制台；其次利用ElMessage报给用户；最后还可以发送API给后端，告诉这里有问题。

    结果：在main入口文件添加全局错误捕获，增强调试能力

## 2. 接口错误统一处理：响应拦截器中统一处理 400/500 等接口错误，给出友好提示

    1、http.interceptors.request.use()创建请求拦截器
    2、http.interceptors.response.use()创建响应拦截器

    我们可以在响应拦截器中，获取后端传来的数据，统一处理各种接口错误，
    甚至还可以处理后端统一result文件里面的各种常见错误，然后通过ElMessage提示用户
...
## 4. 接口超时处理：设置 axios 请求超时时间，超时后给出重试提示
    ...
    1、axios自带了timeout超时
    2、在响应拦截器中，判断是否为超时导致的error
    3、如果是的话，可以弹出一个消息框，让用户选择，是否重试。

## 5. 性能监控：用performance API 监控首屏加载、路由切换耗时，上报分析

    1、封装性能监控工具文件（performanceMonitor.js）
    2、在路由切换中引用，
    3、记得在挂载应用前初始化性能监控

    结果：后续可以直接发送给后端，这里仅仅在页面中展示，而且这个监控页面的工具还可以扩展为Agent开发中的感知层~

# 十、扩展与集成功能（2 个）

核心目标：丰富项目能力，对接常用第三方服务

## 1. 二维码生成 / 解析：集成qrcode/jsqr，实现二维码生成和扫码解析

    1、基于qrcode库，将输入的文本/链接转换为base64格式的二维码图片
    2、通过浏览器getUserMedia API获取摄像头流，渲染到video标签，定时截取视频帧转为Canvas图像数据，再用jsqr解析帧中的二维码内容
    3、边界处理：兼容摄像头权限申请失败、无摄像头设备、解析失败等异常场景。

    结果：封装了一个组件，可以将输入的文本自动转换为二维码

## 2、基于TOTP二次验证

    1、后端发送二维码
    2、前端用谷歌或特定浏览器插件扫描二维码并返回6位验证码
    3、后端绑定该谷歌或特定浏览器，返回成功，并且在后端标记该用户注册了TOTP二次验证
    4、后续该用户只需要 看谷歌或者浏览器插件中不断更新的一次性密码，然后直接拿这个密码去登录即可。

    后端注意IP限流、异常行为检测、验证码多次错误限流等安全措施...

待完成：微信登录、微信支付、地图集成、消息推送...

# 希望你完成的任务...

TODO： 性能数据可视化面板（将封装好的performanceAPI封装成面板，能够被可视化发现）
TODO: 加载状态精细化，列表加载时，骨架屏，逐行渲染，模拟真实数据加载节奏。
TODO： 图片懒加载时，添加“模糊占位->清晰显示”的过渡
TODO： 微交互与场景化：{
1、文章标题滚动到视口时，触发“从左滑入 + 渐显”动画
2、文章段落添加“逐行渐显”动画
}
TODO： 评论区场景：{
1、评论提交，新评论“从下滑入+轻微缩放”动效
2、评论点赞按钮点击，添加“心跳缩放+颜色渐变”动效
}
TODO： 列表场景：{
1、分页切换时，当前页按钮添加脉冲动画。让用户明确感知当前页码。
}
TODO： 完善移动端适配（难度：⭐⭐）{
1、下拉刷新/上拉加载
2、软键盘适配
...
TODO： 实时评论（难度：⭐⭐⭐）{
1、博主登录后，收到新评论时，WebSocket实时推送通知
2、普通用户评论后，实时看到自己的评论出现在列表
3、评论区显示“XX正在输入”的实时状态？（基于WebSocket的心跳包）
}
TODO： 文章场景化功能（难度：⭐⭐）{
1、二维码生成：每篇文章页添加“文章二维码”？
2、TOTP二次验证：集成到登录流程，用户可在“个人中心”开启TOTP，危险操作需TOTP二次验证（删除文章等...）
}
TODO： 用户中心精细化（落地难度：⭐⭐）{
1、不同角色在用户中心看到不同按钮
2、用户编辑个人资料，表单自动保存在localStorage
3、修改密码，实时显示密码强度，并给出优化建议
}
TODO： 自定义组件库封装 （落地难度：⭐⭐⭐）{
把已封装好的通用组件进一步封装成专属组件库，并发布。npm，实现组件复用，顺便打通流程。
}
TODO: 前端测试（⭐⭐⭐）{
1、单元测试：用Vitest测试封装的工具函数
2、组件测试：用Testing Library测试核心组件
3、E2E测试：用Cypress测试核心流程
}

TODO： CI/CD自动化部署{
利用 GitHub Actions/GitLab CI，实现 “提交代码→自动构建→自动部署”：
}

TODO： 性能预算与监控{
配置打包体积预算（比如单个 chunk 不超过 200KB），超过则打包失败；
集成前端监控工具（比如 Sentry），捕获线上错误并报警；
定期分析打包体积（用 rollup-plugin-visualizer），优化大依赖（比如替换体积大的库）。
}
